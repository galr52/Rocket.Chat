...
Accounts.registerLoginHandler(function (loginRequest) {
	...
	if (loginResult && loginResult.profile && loginResult.profile.username) {
		var user = Meteor.users.findOne({
		...
		if (!user) {
			var newUser = {
				name: loginResult.profile.cn || loginResult.profile.name,
			...
			user.name = loginResult.profile.cn || loginResult.profile.name;
			user.active = true;
			user.globalRoles = ['user'];
			user.emails = [{
				address: loginResult.profile.email,
				verified: true
			}];
