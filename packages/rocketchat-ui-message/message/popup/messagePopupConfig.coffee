...
Meteor.startup ->
	Tracker.autorun ->
		...
		messageUsers = RocketChat.models.Messages.find({rid: Session.get('openedRoom'), 'u.username': {$ne: Meteor.user().username}}, {fields: {'u.username': 1, ts: 1, 'u.name': 1}, sort: {ts: -1}}).fetch()
	...
	popupUserConfig: ->
		...
		config =
			...
			getFilter: (collection, filter) ->
				...
				# Gal
				items = _.uniq items, (user) -> return user.username
