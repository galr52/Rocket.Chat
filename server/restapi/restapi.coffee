...
Api.testapiValidateRoomName = (rooms) ->
	for room, i in rooms
		if room.name?
			...
			console.log nameValidation.test
			
			if nameValidation.test room.name
				continue
		throw new Meteor.Error 'invalid-room-record', '[restapi] bulk/updateRoom -> record #' + i + " is invalid"
	return
###
...
Api.addRoute 'bulk/createPrivateRoom', authRequired: true,
	post:
		action: ->
			if RocketChat.authz.hasPermission(@userId, 'create-p')
				try	
					this.response.setTimeout (1000 * @bodyParams.rooms.length)
					Api.testapiValidateRooms @bodyParams.rooms
					ids = []
		...
		action: ->
			...
			if RocketChat.authz.hasPermission(@userId, 'edit-room')
				...
				try
					...
					ids = []
					console.log @bodyParams.rooms
					Meteor.runAsUser this.userId, () =>
						(ids[i] = Meteor.call 'updateRoom', incoming.id, incoming.name, incoming.usernames) for incoming, i in @bodyParams.rooms
					status: 'success', ids: ids # need to handle error
				catch e
